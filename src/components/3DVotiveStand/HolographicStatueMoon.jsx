...
function HolographicStatueMoon({ 
  ...
  const holographicMaterial = useMemo(
    () =>
      ...
      void main() {
        ...
        float glitchStrength = sin(glitchTime) + sin(glitchTime * 2.45) + sin(glitchTime * 5.76) * 1.1;
        glitchStrength /= 1.0;
        glitchStrength = smoothstep(0.3, 1.0, glitchStrength);
        glitchStrength *= 0.0003;
  ...
  useEffect(() => {
    ...
    loader.load("/CyberpunkMaryHeart1.glb", (gltf) => {
      ...
      statue.traverse((child) => {
        if (child.isMesh) {
          ...
          // Skip Object_2 and Object_3
          if (child.name === "Object_2" || child.name === "Object_3") {
            console.log('Preserved original material for:', child.name);
            return;
          }
      ...
      // Add the anchor group to the parent or scene
      if (parentRef && parentRef.current) {
        parentRef.current.add(anchorGroup);
      } else {
    ...
    return () => {
      ...
      if (groupRef.current?.anchor) {
        ...
        // Remove from parent or scene
        if (parentRef && parentRef.current) {
          parentRef.current.remove(groupRef.current.anchor);
        } else {
  ...
  }, [scene, holographicMaterial, loader, onLoad, position, rotation, scale, hover, rotate, parentRef]);
  ...
  useFrame((state, delta) => {
    ...
    //       initialY.current + Math.sin(state.clock.elapsedTime * 0.05) * 0.01;
    //   }
...
export default HolographicStatueMoon;
