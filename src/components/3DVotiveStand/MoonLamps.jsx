...
const MoonScene = ({
  ...
  // Add this new function to handle XCandle1 instancing
  const setupXCandle1Instances = useCallback(() => {
    if (!modelRef?.current) return;

    // Find the original XCandle1 mesh
    let originalXCandle1;
    modelRef.current.traverse((child) => {
      if (child.name === "XCandle1") {
        originalXCandle1 = child;
      }
    });

    if (!originalXCandle1) {
      console.warn("XCandle1 mesh not found in the model");
      return;
    }

    // Create instanced mesh
    const instanceCount = 10; // Adjust number of instances as needed
    const instancedMesh = new THREE.InstancedMesh(
      originalXCandle1.geometry,
      originalXCandle1.material,
      instanceCount
    );
    instancedMesh.name = "XCandle1_instances";

    // Hide original XCandle1
    originalXCandle1.visible = false;

    // Create instances in a circle pattern
    ...
    for (let i = 0; i < instanceCount; i++) {
      ...
      position.set(
        ...
        0, // Keep Y position at 0 or adjust as needed
  ...
  useEffect(() => {
    if (
      ...
      // Keep existing model physics setup
      setTimeout(() => {
        addModelToPhysics();
        setupXCandle1Instances(); // Add this line to create instances
      }, 200);
    }
  }, [modelRef?.current, isPhysicsInitialized.current, setupXCandle1Instances]);
};
