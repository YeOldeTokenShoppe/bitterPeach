...
const getRandomParticlePos = (particleCount, radius = 200) => {
  ...
  for (let i = 0; i < particleCount * 3; i += 3) {
    ...
    // Small stars (80%)
      ...
      sizeMultiplier = Math.random() * 0.005 + 0.001; // Small stars
    }
...
const StarField = ({ count1 = 250, count2 = 1500, is80sMode = false }) => {
  ...
  const [starTextures] = useState(() => {
    ...
    return [loader.load("/star1.png"), loader.load("/star2.png")];
  });
  ...
  const starMaterial1 = useRef(
    new THREE.PointsMaterial({
      ...
      map: starTextures[0], // Use the first loaded texture
    })
  );
  ...
  const starMaterial2 = useRef(
    new THREE.PointsMaterial({
      ...
      depthTest: true,
      blending: THREE.AdditiveBlending,
      map: starTextures[1], // Use the second loaded texture
    })
  );
